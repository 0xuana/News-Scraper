...
- **Coverage audit:** with `scheduled --verify-coverage`, merge all successful request intervals within the latest `INITIAL_BACKFILL_DAYS` every 24 hours, log each gap's bounds as a warning, and page backward from its right edge. A network failure does not advance the checkpoint and is retried after 30 minutes.
